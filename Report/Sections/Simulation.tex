...
To guarantee a viable result of the Monte Carlo analysis, a total of $20.000$ simulations with different values were executed, resulting in the Transfer Functions presented in Figure \ref{fig:ADC_TF_ALLCAPS}.
...
The distribution of the ADC's SNR is represented by a left-skewed distribution, with a mean value of approximately $73.85\si{\decibel}$, validating the value obtained previously.

Regarding the Linearity distribution, this is represented by a normal distribution, equl to the distribution of the capacitor values, with a mean of $14,25bits$, approximately, higher than the $12bits$ resolution of the ADC, which means, the resolution presented is supported by the converter.
